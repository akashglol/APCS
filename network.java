public class network {
    ...
    inputs.
    How it Works:
    1. Initializes array of layers with length of layerSizes array passed from the "SimpleDialogueSystem" class in the main method.
    ...
    set the variable to the inputSize. Otherwise, the variable is set to the size of the previous layer in the layerSizes array.
  ...
  * 2. Iterate through layers array created above, and set each value of the array by calling the output() function for the
    ...
    * 3. Return the array */
    public double[] output(double[] inputs) {
        double[] ans = new double[inputs.length];
        for( int i = 0;i< layers.length;i++){
